...
> **📢 Обновление 2026-05-17:** датасет расширен. **Если ты уже клонировал репо — выполни `git pull`**. Если есть локальные изменения — сначала `git stash`. Smoke test прежний.

## Что внутри

- `data/fitflow.db` — SQLite-база с двумя таблицами (`events`, `feedback`). 107К событий + 1.2К отзывов.
